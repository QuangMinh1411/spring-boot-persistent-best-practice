...
DROP PROCEDURE IF EXISTS FETCH_NICKNAME_GENRE_AND_AGE_BY_GENRE ^;
...
CREATE DEFINER=root@localhost PROCEDURE FETCH_NICKNAME_GENRE_AND_AGE_BY_GENRE(IN p_genre CHAR(20))
BEGIN
    SELECT nickname, age FROM author WHERE genre = p_genre;
    SELECT genre, age FROM author WHERE genre <> p_genre;
END ^;
